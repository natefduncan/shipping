...
from shipping.ups import get_rate as get_ups_rates
from shipping.ups import get_token, get_ups_zones
from shipping.usps import get_rate as get_usps_rates
from shipping.usps import get_usps_zones
...
def create_ups_token():
    token = get_token(UPS_CLIENT_ID, UPS_CLIENT_SECRET)
...
def get_rate_request(from_zip, from_state, to_zip, to_state, ounces, size, date, country="US"):
    return RateRequest(
...
def get_ups(from_zip, from_state, to_zip, to_state, ounces, size, date, new_token, map_dir, ignore_ground):
    rate_request = get_rate_request(from_zip, from_state, to_zip, to_state, ounces, size, date)
    if new_token:
        create_ups_token()
    token = get_ups_token()
    access_token = token["access_token"]
    return [i.to_dict() for i in get_ups_rates(access_token, rate_request, map_dir, ignore_ground)]
...
def ups(from_zip, to_loc, ounces, size, date, new_token, download_maps, map_dir, ignore_ground):
    to_state, to_zip = to_loc.split(",")
    date = dt.datetime.strptime(date, "%Y-%m-%d")
    if download_maps:
        download_ups_maps(from_zip, map_dir)
    rates = get_ups(from_zip, "", to_zip, to_state, ounces, size, date, new_token, map_dir, ignore_ground)
